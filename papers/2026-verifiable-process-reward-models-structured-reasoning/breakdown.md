...
**ZERO defects.** Re-checked Table 2 (the main results) in full against `paper_layout.txt` lines 405–435: all 20 scored model rows × 6 columns (CF / RoBBR-C / RoBBR-NC × Acc / F1) byte-exact — 132 cells, including the 2 all-dash DeepSeek rows (–) and every GRPO/DAPO/VPRM/SFT cell. All §4.4 takeaways recompute from the cells: vs Llama-3.1-405B +19.5 Acc / +31.2 F1 (87.9−68.4 / 76.7−45.5) ✓; vs GPT-OSS-120B +20.8 / +26.9 ✓; GRPO-VPRM over GRPO +6.4 Acc / +6.5 F1 ✓; GRPO>DAPO both with and without VPRM (87.9>79.2; 81.5>76.8) ✓. The abstract's "20% higher F1" mislabel is the paper's own (Accuracy gap mislabeled as F1) — flagged ⚠ above, transcribed verbatim, not a breakdown defect. Confirms scramble-modes meta-finding for RL-reward / clinical-NLP methods papers: zero cell typos. No edits required.
